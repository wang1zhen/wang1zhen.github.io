...
interpolation
tscale=oversample                     # 使得插值更平滑

# ========== 播放控制 ==========
keep-open=always                      # 播放完暂停
...
# ========== 界面设置 ==========
osc=no                                # 禁用默认OSC (使用 ModernZ)
osd-bar=no
border=no
autofit-larger=80%x80%

# ========== Anime4K 智能加载策略 ==========
# 定义一个条件配置：只有分辨率 <= 1080P (高度 <= 1080) 时才启用 Anime4K
[Anime4K-Auto]
...
### audio-osc - OSC 可见性智能控制 {#audio-osc-osc-可见性智能控制}
...
-   播放纯音频文件时，OSC 始终显示（方便控制进度和音量）
-   播放视频文件时，OSC 恢复自动隐藏模式（配合 ModernZ 的 bottomhover）

这样可以避免播放音频文件时需要移动鼠标才能看到控制界面的不便。


#### 配置方法 {#配置方法}

无需额外配置，脚本会在每次文件加载时自动检测并调整 OSC 显示模式。


## SVP 补帧配置 {#svp-补帧配置}

SVP (SmoothVideo Project) 是实时视频补帧软件，可将低帧率视频提升至高帧率。使用 SVP 需要在 `mpv.conf` 中添加以下配置：

```cfg
# ========== SVP 补帧配置 ==========
hwdec=auto-copy                     # 使用兼容 Vapoursynth 的硬件解码
hwdec-codecs=all
hr-seek-framedrop=no                # 修复音频同步
no-resume-playback                  # 禁用播放进度记忆（与 SVP 冲突）
```

配合已有的 mpvSockets 脚本，无需额外配置 IPC Socket。
